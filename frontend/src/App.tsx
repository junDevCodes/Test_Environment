...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      try {
        delete (window as any).__openKeyModal;
      } catch {
        /* noop */
      }
    };
  ...
  const handleSaveKey = () => {
    ...
    api.post('/api/config/gemini', { api_key: apiKey.trim() })
      .then(() => {
        setNeedsKey(false);
        try { window.dispatchEvent(new CustomEvent('gemini:updated', { detail: { set: true } })); } catch {}
      })
